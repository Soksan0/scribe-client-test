    ...
    try {
      ...
      if (!route.projectId) {
        ...
        if (route.section === "rules") {
          const [ruleData, suggestionData, relationshipData] = await Promise.all([api(`/api/projects/${route.projectId}/rules`), api(`/api/projects/${route.projectId}/rules/suggestions`), api(`/api/projects/${route.projectId}/relationships`)]);
          setRules(ruleData); setSuggestions(suggestionData); setRelationships(relationshipData);
        }
        if (route.section === "exports") setExports(await api(`/api/projects/${route.projectId}/exports`));
      }
    } catch (reason) {
...
function ReviewOnlyPanel({ active, rationale, setRationale, disposition, busy }: any) {
  ...
  return <div className="review-only-panel"><div className="review-only-heading"><span>Needs confirmation</span><h3>{guidance.problem}</h3><p>{guidance.risk}</p></div><div className="strategy-card"><strong>Recommended cleaning strategies</strong><ul>{guidance.strategies.map((strategy: string) => <li key={strategy}>{strategy}</li>)}</ul></div><label className="rationale-field">Decision note<textarea value={rationale} onChange={(event) => setRationale(event.target.value)} placeholder="Choose a rationale below or write your own evidence-backed note." /></label><div className="review-actions">{actions.map((action) => <button key={action.value} type="button" className={action.primary ? "primary-button review-action" : "review-action"} disabled={busy} onClick={() => rationale.trim() === action.rationale ? disposition(action.value, rationale) : setRationale(action.rationale)}><strong>{action.title}</strong><span>{action.body}</span><small>{rationale.trim() === action.rationale ? "Click to save this review" : "Click to use this rationale"}</small></button>)}</div></div>;
}

function IssuesView({ findings, total, page, setPage, status, setStatus, active, preview, select, decide, disposition, undo, detailsOpen, closeDetails, openDetails, busy }: any) {
